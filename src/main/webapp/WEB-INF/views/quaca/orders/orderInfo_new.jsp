...
<script type="text/javascript">
	...
	function ocOrderHtml(data){
		...
		for(var i = 0; i < data.length; i++){
			...
			html += 							'주문 접수';
			html += 						'</button>';
			html +=							'<button type="button" class="btn btn-dark OrderCancel" onclick="orderUpdate(${userVO.storeId},\''+data[i].UserPayId+'\','+'\'CUP'+'\');">';
			html +=								'주문 취소';
			html +=							'</button>';
			html +=						'</div>'
			html += 				'</div>';
	...
	function rcOrderHtml(data){
		...
		for(var i = 0; i < data.length; i++){
			...
			html +=						'<button type="button" class="btn btn-dark" onclick="orderUpdate(${userVO.storeId},\''+data[i].UserPayId+'\','+'\'CUP'+'\');">';
			html +=							'주문 취소';
			html +=						'</button>';
			html +=					'</div>';
			html +=				'</div>';
