...
<main id="js-page-content" role="main" class="page-content">
	<ol class="breadcrumb page-breadcrumb">
        ...
        <li class="breadcrumb-item">공지 사항</li>
			...
			<div id="panel-1" class="panel">
				<div class="panel-container show">
					<div class="panel-content">
						...
						<div class="row">
							<div class="col-xl-12">
								<button class="btn btn-primary" data-toggle="modal" data-target="#boardInsertModal">등록하기</button>
							</div>
						</div>
							...
							<div class="modal-dialog modal-lg" role="document">
								<div class="modal-content">
									<div class="modal-header center-block">
										<h1 class="modal-title fa-3x"><b>공지사항 등록</b></h1>								  
									</div>
						...
						<div class="modal fade" id="boardUpdateModal" tabindex="-1" role="dialog" aria-hidden="true">
							...
							<div class="modal-dialog modal-lg" role="document">
								<div class="modal-content">
									<div class="modal-header center-block">
										<h1 class="modal-title fa-3x"><b>공지사항 수정</b></h1>								  
									</div>
									<div class="modal-body" id="modal_body">
										<ul class="list-group">
											<li class="list-group-item">
...
<script type="text/javascript">
	...
	//1-1.[게시판 목록 호출] - 공지사항 목록 가져오기
	function boardList(){
		$.ajax({
			url : express_docker+"notice/boardlist",
			type : "POST",
			data : {
	...
	//1-2.[게시판 목록 호출] - 공지사항 목록 table html (data = true)
	...
	function boardListBasisHtml(){
		...
		html += 	'<tbody>';
		html += 		'<tr class="text-center" >';
		html += 			'<td colspan="4"> 데이터가 존재하지 않습니다.</td>';
		html += 		'</tr>';
		html +=    '</tbody>';
		html += '</table>';
	...
	//2.[게시판 상세정보 호출] - 공지사항 정보 호춣 
	function boardInfo(sid){
		$.ajax({
			url : express_docker+"notice/boardselect",
			type : "POST",
			data : {
				StoreId : "${userVO.storeId}",
				SID : sid
	...
	//3.[게시판 상세정보 수정] - 공지사항 정보 수정 
	function boardUpdate(){
		if(validate('boardUpdateModal')){
			$.ajax({
				url : express_docker+"notice/boardupdate",
				type : "POST",
				data : {
					StoreId : "${userVO.storeId}",
					SID : $("#boardUpdateModal").find("#sid").val(),
					Title : $("#boardUpdateModal").find("#title").val(),
					Contents : $("#boardUpdateModal").find("#contents").val(),
					InsertNm : $("#boardUpdateModal").find("#insertNm").val(),
	...
	//4.[게시판 정보 등록] - 공지사항 등록 처리
	function boardInsert(){
		if(validate('boardInsertModal')){
			$.ajax({
				url : express_docker+"notice/boardinsert",
				type : "POST",
				data : {
					StoreId : "${userVO.storeId}",
					Title : $("#boardInsertModal").find("#title").val(),
					Contents : $("#boardInsertModal").find("#contents").val(),
					InsertNm : $("#boardInsertModal").find("#insertNm").val(),
				},success : function(data){
					if(data.success){
						alert("등록 되었습니다.");
						$("#boardInsertModal").modal("hide");
						boardReSet("boardInsertModal");
						boardList();
					}else{
						alert("등록 실패 했습니다.");
					}
	...
	function validate(id){
		...
		if($("#"+id).find("#title").val() == ""){
			validateChk = false;
			alert("제목을 입력해 주세요.");
			$("#"+id).find("#title").focus();
		}else if($("#"+id).find("#insertNm").val() == ""){
			validateChk = false;
			alert("등록자를 입력해 주세요.");
			$("#"+id).find("#insertNm").focus();
		}else if($("#"+id).find("#contents").val() == ""){
			validateChk = false;
			alert("내용을 입력해 주세요.");
			$("#"+id).find("#contents").focus();
		}
		return validateChk;
	}
	...
	function boardReSet(id){
		$("#"+id).find("#title").val("");
		$("#"+id).find("#insertNm").val("");
		$("#"+id).find("#contents").val("");
	}
