						...
						<div class="form-group row">
							...
							<div class="col-12 col-lg-6">
								<div class="input-group">
									...
									<input type="hidden" class="form-control " id="eventCd" name="eventCd" value="">
								</div>
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-12 col-lg-6">
								<div class="input-group">
									<select class="form-control " id="eventType" name="eventType" disabled="disabled">
...
<script type="text/javascript">
	...
	//1-1.[행사 정보 호출] - 행사 정보 불러오기 
	function eventInfo(){
		$.ajax({
			url : express_docker+"store/eventListDetail",
			type : "POST",
			data : {
				...
				if(data.success){
					$("#eventCd").val(data.info.EventCd);
					$("#eventNm").val(data.info.EventNm);
					$("#startDt").val(data.info.StartDt);
					$("#endDt").val(data.info.EndDt);
					$("#eventType").val(data.info.EventType);
					$('input[name="useYn"]').each(function(){
						if($(this).val().trim() == data.info.UseYn.trim()){
							this.checked = true;
						}
					});
					if(data.info.EventType == "D"){
		    			$("#eventTypeDiv").html(discountHtml());
		    			
		    			$('input[name="saleType"]').each(function(){
							if($(this).val().trim() == data.info.SaleType.trim()){
								this.checked = true;
							}
						});
		    			$("#saleValue").val(data.info.SaleValue);
		    			
		    			categoryList(data.info.CateCd, data.info.MenuId);
		    			
		    			
		    		}else{
		    			$("#eventTypeDiv").html(frequencyHtml());
		    			frequencyInfo(data.info.EventCd);
		    		}
	...
	//1-2.[행사 정보 호출] - (EvendType = 'F') 프리퀀시 정보 불러오기 
	function frequencyInfo(eventCd){
		$.ajax({
			url : express_docker+"store/eventfrequencyList",
			type : "POST",
			data : {
				EventCd : eventCd
			},success : function(data){
				if(data.success){
					$("#optionA").val(data.info.OptionA);
					$("#cntA").val(data.info.CntA);
					$("#optionB").val(data.info.OptionB);
					$("#cntB").val(data.info.CntB);
					$("#optionC").val(data.info.OptionC);
					$("#cntC").val(data.info.CntC);
					$("#present").val(data.info.Present);
				}else{
					alert("정보를 불러오기 실패 했습니다.");
				}
			},
	...
	function discountHtml(){
		...
		html += 			'<select class="form-control " id="cateCd" name="cateCd"  disabled="disabled">';
		html += 			'</select>';
		html += 		'</div>';
		html += 	'</div>';
		html += '</div>';
		...
		html += 	'<div class="col-12 col-lg-6">';
		html += 		'<div class="input-group">';
		html += 			'<select class="form-control " id="menuId" name="menuId"  disabled="disabled">';
		html += 			'</select>';
		html += 		'</div>';
		html += 	'</div>';
		html += '</div>';
	...
	function frequencyHtml(){
		...
		html += 	'<div class="col-12 col-lg-6">';
		html += 		'<div class="input-group">';
		html += 			'<input class="form-control " type="text" id="present" name="present" value="" placeholder="증성">';
		html += 		'</div>';
		html += 	'</div>';
		html += '</div></br>';
		return html;
	...
	function categoryList(cateCd, menuId){
		$.ajax({
			...
			success : function(data){
				if(data.success){
					$("#cateCd").html(categoryOptionHtml(data.info, cateCd));
					// 메뉴 리스트 호출
					menuList(menuList($("#cateCd option:selected").val(), menuId));
				}else{
	...
	function categoryOptionHtml(data, cateCd){
		var html;
	...
	function menuList(cateCd, menuId){
		$.ajax({
	...
	function menuOptionHtml(data, menuId){
		var html;
		for(var i = 0; i < data.length; i++){
			html += '<option value="'+data[i].MenuId+'"';
			html += menuId == data[i].MenuId ? 'selected' : '';
			html += '>';
	...
	function eventSave(){
		...
		if(validate()){
			$.ajax({
				url : express_docker+"store/eventUpdate",
				type : "POST",
				data : {
					StoreId : "${userVO.storeId}",
					EventCd : eventCd,
	...
	//1-2.[행사 정보 수정] - 프리퀀시 옵션 저장 처리 
	function frequencySave(eventCd){
		$.ajax({
			url : express_docker+"store/eventfrequencyUpdate",
			type : "POST",
			data : {
				StoreId : "${userVO.storeId}",
				EventCd : eventCd,
